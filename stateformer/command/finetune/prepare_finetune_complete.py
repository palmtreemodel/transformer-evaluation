...
class params:
    fields = ['static', 'inst_pos_emb', 'op_pos_emb', 'arch_emb', 'byte1', 'byte2', 'byte3', 'byte4', 'arg_info']
...
def get_ds_loc(loc_dict, address, funcname):
    for var in loc_dict[funcname]:
        if address in [int(i, 16) for i in loc_dict[funcname][var]['addresses']]:#+4194304
...
def get_arg_info(loc_dict, funcname):
    ...
    while len(arg_list) < 3:
        arg_list.append('##')
    arg_list = [arg_type.replace(' ', '_') for (order, arg_type) in arg_list]
...
print(input_dir, stack_dir, output_dir)
# arch_input = args.arch
# output_dir = 'data-src/finetune/x86-64-cross-bin'
# input_dir = '/mnt/sata/lian/NumeracyInsEmbedding/data/binaries/otherbins'
# stack_dir = '/mnt/sata/lian/NumeracyInsEmbedding/data/stacks'
...
for filename in file_list:
    ...
    with open(filename, 'rb') as f:
        ...
        for segment_idx in range(elffile.num_segments()):
            segment=elffile.get_segment(segment_idx)
            seg_head=segment.header
            if seg_head.p_type == 'PT_LOAD' and seg_head.p_offset==0:
                base_addr = seg_head.p_vaddr

        print(filename, 'base addr', base_addr)
        ...
        for CU in dwarf.iter_CUs():
            ...
            for func in function_reps:
                ...
                if not random.random() < 0:
                    ...
                    train_file[params.fields[8]].write(func['name'] + ' ' + ' '.join(arg_info) + '\n')
                    ...
                    valid_file[params.fields[8]].write(func['name'] + ' ' + ' '.join(arg_info) + '\n')
