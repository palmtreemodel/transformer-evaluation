...
class Function:
    ...
    @classmethod
    def load(self, json_line:dict):
        '''
        gcc -S format compatiable
        '''
        lines = [i for item in json_line["BasicBlocks"] for i in item]
        label, labels = None, {}
        insts:List[Instruction] = []
        blocks:List[BasicBlock] = []
        meta = {}
        addrs = []
        meta['name'] = json_line["Name"]
        meta['offset'] = "1024"
        meta['file'] = json_line["file"]
        for line in lines:
...
def create_functions(functionlist, bin_file, func_file = None) -> List[Function]:
    # prepare data    
    functions:dict[int,Function] = {}

    for func in tqdm.tqdm(functionlist.values()):
        func['file'] = bin_file
        fn = Function.load(func)
...
def load_asmfiles(datahome, subdirs:List[str], b_save = True):
    inpath = datahome
    inpath = subdirs[0] + "\\" + subdirs[1]
    # for dir in subdirs:
    #     inpath = Path(inpath)
    funcs = []
    function_list = json.load(open(inpath,"r"))
    funcs += create_functions(function_list, inpath)
