...
dirs = ['C:\\Users\\Zixiang\\jTrans\\x86-64']
sub_dirs = glob.glob(f'{dirs[0]}\\*')[:50]
...
if __name__ == '__main__':
    ...
    for subdir in sub_dirs:
        for bin in os.listdir(subdir):
            bindirs.append("{}_{}".format(subdir, bin))
            print("Processing {}\{}".format(subdir, bin))
            funcs = load_asmfiles(ASM_PATH, [subdir, bin], False)        
            func_all+=funcs
            funcss['{}_{}'.format(subdir, bin)] = funcs
    ...
    all_seg = []
    # for t in dirs:
    for c in combinations_with_replacement(bindirs, 2):
        folder1 = c[0].split("\\")[-1].split("_")[0].split("-")[:-1]
        folder2 = c[1].split("\\")[-1].split("_")[0].split("-")[:-1]
        opt1 = c[0].split("\\")[-1].split("_")[0].split("-")[-1]
        opt2 = c[1].split("\\")[-1].split("_")[0].split("-")[-1]
        bin1 = c[0].split("\\")[-1].split("_")[1]
        bin2 = c[1].split("\\")[-1].split("_")[1]
        if c[0] == c[1] or folder1 != folder2 or opt1 == opt2 or bin1 != bin2:
            continue
        ids, seg = gen_features(funcss[c[0]], funcss[c[1]], '{}/{}'.format( c[0], c[1]), type='liner', b_num=True)
        all_ids += ids
        all_seg += seg
    ...
    if valid_size % 2 != 0:
        valid_size -= 1
    create_datasets("./traindata/train-{}.tfrec".format(train_size), all_ids[:train_size], all_seg[:train_size])
    create_datasets("./traindata/validate-{}.tfrec".format(valid_size), all_ids[train_size:train_size + valid_size], all_seg[train_size:train_size + valid_size])
